export type WeatherContext = {
  ...
  retrievedAt: string;
  confidence: number;
  status: "LIVE_OR_NEAR_LIVE" | "ESTIMATED";
};
...
export async function getCurrentWeather(latitude: number, longitude: number): Promise<WeatherContext> {
  ...
  const data = (await response.json()) as {
    current?: { temperature_2m?: number; precipitation?: number; wind_speed_10m?: number; time?: string };
  ...
  return {
    ...
    rainfallMm: data.current?.precipitation ?? null,
    precipitationProbability: index >= 0 ? data.hourly?.precipitation_probability?.[index] ?? null : null,
